  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400">{overall.label ?? '—'}</div>
      </div>
      ...
      <div className="space-y-2">
        {per_article.length === 0 ? (
          ...
          <div key={i} className="p-3 bg-gray-50 dark:bg-[#051025] rounded-lg">
            ...
            {a.sentiment && <div className="text-xs mt-1 text-gray-600 dark:text-gray-300">Sentiment: {a.sentiment.label} ({(a.sentiment.score ?? 0).toFixed(2)})</div>}
          </div>
        ))}
      </div>
    </div>
